...
export function AuthGate({ children }) {
  ...
  if (user && userStatus === 'pending') {
    return (
      <div style={{ minHeight: '100vh', background: `linear-gradient(180deg,${T.bg} 0%,#12110f 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Cormorant Garamond',serif" }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>⏳</div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: T.ac, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Compte en attente</h1>
          </div>
          <div style={{ ...sCard, cursor: 'default', padding: 28, textAlign: 'center' }}>
            <p style={{ color: T.tx, fontSize: 15, lineHeight: 1.6, marginBottom: 16 }}>
              Ton compte (<strong>{user.email}</strong>) est en attente de validation par l'administrateur.
            </p>
            <p style={{ color: T.mu, fontSize: 13, marginBottom: 24 }}>
              Tu recevras l'accès une fois ton compte approuvé. Rafraîchis la page après approbation.
            </p>
    ...
    return (
      <div style={{ minHeight: '100vh', background: `linear-gradient(180deg,${T.bg} 0%,#12110f 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Cormorant Garamond',serif" }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>🚫</div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#e88', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Accès refusé</h1>
          </div>
          <div style={{ ...sCard, cursor: 'default', padding: 28, textAlign: 'center' }}>
  ...
  const linkStyle = {
    color: T.ac, cursor: 'pointer', fontSize: 13, textDecoration: 'underline',
    ...
    <div style={{ minHeight: '100vh', background: `linear-gradient(180deg,${T.bg} 0%,#12110f 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Cormorant Garamond',serif" }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ ...sCard, cursor: 'default', padding: 28 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.tx, marginBottom: 20, textAlign: 'center' }}>
            {screen === 'login' ? 'Connexion' : screen === 'signup' ? 'Créer un compte' : 'Mot de passe oublié'}
          </h2>

          {error && (
            <div style={{ background: '#9b4d4d22', border: '1px solid #9b4d4d44', borderRadius: 4, padding: '8px 12px', marginBottom: 12, color: '#e88', fontSize: 13 }}>
              {error}
            </div>
          )}
          {msg && (
            <div style={{ background: '#5a8f6e22', border: '1px solid #5a8f6e44', borderRadius: 4, padding: '8px 12px', marginBottom: 12, color: '#8ec8a0', fontSize: 13 }}>
              {msg}
            </div>
          )}
          ...
          {screen === 'signup' && (
            <p style={{ fontSize: 12, color: T.mu, marginBottom: 12, lineHeight: 1.5 }}>
              Ton compte sera soumis à validation par l'administrateur avant de pouvoir accéder à l'application.
            </p>
          )}

          <button style={{ ...sBtnA, width: '100%', fontSize: 16, padding: '12px 20px', marginTop: 4 }} onClick={screen === 'login' ? handleLogin : screen === 'signup' ? handleSignup : handleReset}>
            {screen === 'login' ? 'Se connecter' : screen === 'signup' ? 'Envoyer la demande' : 'Envoyer'}
          </button>

          <div style={{ marginTop: 16, textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {screen === 'login' && (
              <>
                <button style={linkStyle} onClick={() => { setScreen('signup'); setError(''); }}>Créer un compte</button>
                <button style={linkStyle} onClick={() => { setScreen('reset'); setError(''); }}>Mot de passe oublié ?</button>
              </>
            )}
            {screen === 'signup' && (
              <button style={linkStyle} onClick={() => { setScreen('login'); setError(''); }}>Se connecter</button>
            )}
            {screen === 'reset' && (
              <button style={linkStyle} onClick={() => { setScreen('login'); setError(''); }}>Retour</button>
            )}
          </div>
        </div>
      </div>
